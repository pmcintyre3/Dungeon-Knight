        ...
        EntityPlayer = ig.Entity.extend({
            ...
            update: function () {
                ...
                if(!this.swinging) {
                    ...
                    this.minTheta = 225 * (Math.PI / 180);
                    this.maxTheta = 315 * (Math.PI / 180);
                    if(this.theta == 0){
                        this.theta = this.minTheta;
                    }
                    if(this.flip == false){
                        this.pos.x += this.rope.size.x * Math.cos(this.theta);
                        this.pos.y -= this.rope.size.y * Math.sin(this.theta);
                        this.theta += 1.5 * (Math.PI / 180);
                    }
                    else{
                        this.pos.x -= this.rope.size.x * Math.cos(this.theta);
                        this.pos.y -= this.rope.size.y * Math.sin(this.theta);
                    ...
                    if(this.theta == this.maxTheta)
                        this.flip = true;
                    else if(this.theta == this.minTheta)
                        this.flip = false;
                    ...
                    if(ig.input.pressed('jump')){
                        if(this.flip == false)
                            this.jumpOffRope('right');
                        else
                            this.jumpOffRope('left');
                    }
                }
            ...
            afterMoveAndCollide: function () {
                ...
                if(this.swinging && ig.input.state('jump') && this.atRightSideRope()){
                    this.jumpOffRope('right');
                }
                else if(this.swinging && ig.input.state('jump') && this.atLeftSideRope()){
                    this.jumpOffRope('left');
                }
            ...
            checkForRope: function() {
                this.onRope = false;
                this.size.y += 1;
                this.size.x += 1;
                var ropes = ig.game.getEntitiesByType(EntityRope);
                for (var i = 0; i < ropes.length; i++) {
                    if (this.touches(ropes[i])) {
                        this.rope = ropes[i];
                        this.onRope = true;
                        this.getOnRope();
                        break;
            ...
            atRightSideRope: function () {
                return this.pos.x + this.size.x - 2 <= (this.rope.pos.x + this.rope.size.x);
            },
            atLeftSideRope: function () {
                return this.pos.x + this.size.x - 2 <= this.rope.pos.x;
            },
            getOnRope: function () {
                this.pos.x = this.rope.pos.x + this.ladder.size.x / 2 - this.size.x / 2;
                this.friction.y = 600;
                this.gravityFactor = 0;
                this.accel.x = 0;
                this.vel.x = 0;
            ...
            jumpOffRope: function (dir) {
                if (!this.canMoveInDir(dir))
                    return;
                this.pos.x += ig.game.collisionMap.tilesize * (dir == 'left' ? -1 : 1);
                this.pos.y = this.last.y;
                this.swinging = false;
                this.vel.x = this.maxVel.x * (dir == 'left' ? -1 : 1);
                this.gravityFactor = 1;
                this.friction.y = 0;
                this.checkForRope();
            },
